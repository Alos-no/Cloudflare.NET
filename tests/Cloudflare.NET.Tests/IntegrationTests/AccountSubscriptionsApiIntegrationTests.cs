...
public class AccountSubscriptionsApiIntegrationTests : IClassFixture<CloudflareApiTestFixture>
{
  ...
  public async Task ListAccountSubscriptionsAsync_ReturnsSubscriptions()
  {
    ...
    try
    {
      result = await _sut.ListAccountSubscriptionsAsync(accountId);
    }
    ...
    _output.WriteLine($"Found {result.Count} subscriptions");
  }

  /// <summary>I02: Verifies that listing returns empty collection when no subscriptions exist.</summary>
  [IntegrationTest]
  public async Task ListAccountSubscriptionsAsync_WhenEmpty_ReturnsEmptyList()
  {
    // Arrange
    var accountId = _settings.AccountId;
  ...
  /// <summary>I03: Verifies that subscriptions have valid state property.</summary>
  [IntegrationTest]
  ...
  /// <summary>I04: Verifies that subscriptions have rate plan when applicable.</summary>
  [IntegrationTest]
  ...
  /// <summary>I06: Verifies that subscription frequency is populated.</summary>
  [IntegrationTest]
  ...
  /// <summary>I07: Verifies that subscription period dates are returned when available.</summary>
  [IntegrationTest]
  public async Task ListAccountSubscriptionsAsync_ReturnsPeroidDates()
  {
    // Arrange
    var accountId = _settings.AccountId;
  ...
  /// <summary>I08: Verifies that subscription component values are returned when available.</summary>
  [IntegrationTest]
  ...
  [IntegrationTest]
  public async Task UpdateAccountSubscriptionAsync_NonExistent_ThrowsNotFound()
  {
    // Arrange
    var accountId = _settings.AccountId;
    ...
    try
    {
      await _sut.UpdateAccountSubscriptionAsync(accountId, nonExistentSubId, request);
  ...
  [IntegrationTest]
  public async Task DeleteAccountSubscriptionAsync_NonExistent_ThrowsNotFound()
  {
    // Arrange
    var accountId = _settings.AccountId;
    ...
    try
    {
      await _sut.DeleteAccountSubscriptionAsync(accountId, nonExistentSubId);
      _output.WriteLine("Unexpectedly succeeded - subscription might exist or API behavior differs");
    }
    catch (HttpRequestException ex) when (ex.StatusCode == System.Net.HttpStatusCode.Forbidden)
    {
  ...
  [IntegrationTest]
  public async Task CreateAccountSubscriptionAsync_InvalidRatePlan_ThrowsError()
  {
    // Arrange
    var accountId = _settings.AccountId;
    ...
    try
    {
      await _sut.CreateAccountSubscriptionAsync(accountId, request);
  ...
  public async Task UpdateAccountSubscriptionAsync_MalformedId_ThrowsError()
  {
    ...
    try
    {
      await _sut.UpdateAccountSubscriptionAsync(accountId, malformedId, request);
  ...
  public async Task ListAccountSubscriptionsAsync_SubscriptionsHaveVariousStates()
  {
    ...
    stateGroups.Should().NotBeEmpty("account should have at least one subscription state if any subscriptions exist, or be empty");
  }
